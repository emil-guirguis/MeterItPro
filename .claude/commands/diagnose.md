...
|---|-------|---------------|--------|
| 1 | **Wrangler secrets vs .dev.vars** | `cat client/api/.dev.vars`, `npx wrangler secret list` (prod) — compare key names and values | |
| 2 | **Root .env overriding service .env** | `cat .env`, `cat sync/api/.env`, `cat sync/frontend/.env`, `cat client/api/.env` — check for duplicate keys | |
| 3 | **VS Code launch.json env override** | `cat .vscode/launch.json` — look for `env` blocks overriding vars set in .env files | |
| 4 | **Stale dist/ from missing clean** | Check `ls sync/api/dist/ -lt` (newest files), compare timestamps to source edits. Check `package.json` for `clean` script using rimraf | |
...
| 7 | **MUI data-* on wrapper not root** | `grep -r "data-testid" sync/frontend/src` — verify attribute is on the MUI component, not a wrapping `<div>` | |
...
| Root .env override           | ⚠️ Suspected  | DB_URL defined in both root and sync/api |
