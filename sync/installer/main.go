...
func dockerLogin(cfg bootstrapData) {
	if cfg.GithubOwner == "" || cfg.GithubToken == "" {
		fmt.Println("  ⚠ No GitHub credentials in config — skipping ghcr.io login.")
		fmt.Println("    If pull fails, set github_owner and github_token on the sync server in the client site.")
		return
	}
	...
	if err := cmd.Run(); err != nil {
		...
		ok("Logged in to ghcr.io as " + cfg.GithubOwner)
	}
}
