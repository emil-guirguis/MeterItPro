// Customer entity store — read-only. qb_customer is synced from QuickBooks, so
...
const READ_ONLY = 'Customers are synced from QuickBooks and cannot be edited here.';
...
const customerService = {
  async getAll(params?: any) {
    ...
    const qs = q.toString();
    const data = await parse(await fetch(`${API_BASE_URL}/customers${qs ? `?${qs}` : ''}`, { headers: authHeaders() }));
    return { items: data.data?.items || [], total: data.data?.total || 0, hasMore: false };
  },
  async getById(id: string) {
    const data = await parse(await fetch(`${API_BASE_URL}/customers/${id}`, { headers: authHeaders() }));
    return data.data;
  },
...
export const useCustomerStore = createEntityStore<Customer & { id: string }>(customerService as any, {
  name: 'customer',
  cache: { ttl: 5 * 60 * 1000, maxAge: 30 * 60 * 1000 },
});

export const useCustomers = createEntityHook(useCustomerStore);
